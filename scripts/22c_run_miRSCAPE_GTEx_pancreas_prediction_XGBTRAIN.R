...
# Source miRSCAPE helper functions only.
# We do not use the original miRSCAPE() function here because newer xgboost versions
# changed the wrapper interface.
...
if (run_mode == "candidate") {
  ...
  if (length(keep_candidates) == 0) {
    stop("Candidate mode requested, but no available candidates were found.")
  }

  bulk_mirna <- bulk_mirna[keep_candidates, , drop = FALSE]
  message("Running candidate mode only for:")
  print(rownames(bulk_mirna))
}

message("Final dimensions used by xgb.train miRSCAPE:")
print(dim(bulk_mrna))
print(dim(bulk_mirna))
print(dim(target_mrna))
...
message("Running xgb.train miRSCAPE prediction...")
set.seed(123)
...
pred_mirna_by_sample <- miRSCAPE_xgbtrain(
  ...
  bulkmiRNA = bulkmiRNA,
  scmRNA = scmRNA,
  nrounds = ifelse(run_mode == "candidate", 30, 30),
  mode_label = run_mode
)
...
cat("\nDone: xgb.train miRSCAPE GTEx pancreas prediction completed.\n")
