...
message("Downloading TCGA-PAAD miRNA data with files.per.chunk = 1...")
GDCdownload(
  query_mirna,
  method = "api",
  files.per.chunk = 1,
...
message("Trying GDCprepare for miRNA...")
mirna_se <- tryCatch(
  {
    GDCprepare(query_mirna, directory = gdc_mirna_dir)
  },
  error = function(e) {
    message("GDCprepare failed for miRNA. Falling back to direct txt parsing.")
    message(e$message)
    return(NULL)
  }
)

extract_mirna_from_SE <- function(se_obj) {
  if (!inherits(se_obj, "SummarizedExperiment")) {
    stop("miRNA object is not a SummarizedExperiment.")
  }
  
  message("miRNA assay names:")
  print(assayNames(se_obj))
  
  assay_names <- assayNames(se_obj)
  
  if ("reads_per_million_miRNA_mapped" %in% assay_names) {
    mat <- assay(se_obj, "reads_per_million_miRNA_mapped")
  } else if ("read_count" %in% assay_names) {
    mat <- assay(se_obj, "read_count")
  } else {
    mat <- assay(se_obj, 1)
  }
  ...
  if (!is.na(id_col)) {
    rownames(mat) <- as.character(meta[[id_col]])
  }
...
extract_mirna_from_txt <- function(gdc_dir, query_results) {
  files <- list.files(gdc_dir, recursive = TRUE, full.names = TRUE)
  
  files <- files[
    grepl("mirnas|miRNA|mirna", basename(files), ignore.case = TRUE) &
  ...
  idx <- match(file_map$file_name, query_results$file_name)
  
  file_map$sample_id <- query_results$cases[idx]
  
  missing_sample <- is.na(file_map$sample_id)
  
  if (any(missing_sample)) {
    message("Some downloaded files could not be matched by file_name.")
    message("Trying partial file_name matching.")
    
    for (i in which(missing_sample)) {
      hit <- grep(file_map$file_name[i], query_results$file_name, fixed = TRUE)
      if (length(hit) == 1) {
        file_map$sample_id[i] <- query_results$cases[hit]
      }
    }
  }
  ...
  value_col <- intersect(
    c("reads_per_million_miRNA_mapped", "read_count", "RPM", "rpm"),
    colnames(one)
  )[1]
  ...
  all_ids <- unique(unlist(lapply(value_list, names)))
...
if (!is.null(mirna_se)) {
  ...
  mirna_mat <- extract_mirna_from_SE(mirna_se)
} else {
  mirna_mat <- extract_mirna_from_txt(gdc_mirna_dir, mirna_results)
}
...
# ---------- Match with target β-cell pseudo-bulk ----------
...
cat("\nDone: robust TCGA-PAAD Model2 input prepared.\n")
